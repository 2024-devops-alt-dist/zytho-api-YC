...
export const getBeers = async (req: Request, res: Response) => {
  try {
    const data = await pool.query(
      `SELECT beers.name, beers.price, beers.description, beers.abv, beers.color, breweries.name AS brewery
        FROM beers
        JOIN breweries
...
export const getBeerById = async (req: Request, res: Response) => {
  try {
    ...
    const beer = await pool.query(
      `SELECT beers.*, breweries.name AS brewery_name, breweries.country AS country, categories.name AS category
        FROM beers 
        JOIN breweries
        ON beers.brewery_id = breweries.brewery_id
        JOIN beer_has_category
...
export const createBeer = async (req: Request, res: Response) => {
  try {
    ...
    const newBeer = await pool.query(
      ...
      [name, price, description, color, abv, picture_url, production_date, brewery_id]
    );
...
export const updateBeer = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, price, description, color, abv, picture_url, date } = req.body;
